...
private:
    ...
    void fnSearchAuditLog();  // 搜索审计日志（全局搜索+分页）
    void fnDisplaySearchResults(const std::vector<std::map<std::string,std::string>>& results);
    int getTableNameIndex(const std::string& dbName);
    void updatePageLabel();  // 更新页码显示
...
private:
    ...
    static const int PAGE_SIZE = 100;  // 每页显示记录数
};
