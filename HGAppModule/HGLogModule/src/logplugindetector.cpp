...
LogPluginDetector::LogPluginDetector(QObject* parent)
{
    _Plugin_Name="Log";
}
std::string LogPluginDetector::pluginName()
{
    std::string name=_Plugin_Name.toStdString();
    return name;
}
QWidget* LogPluginDetector::getWidget()
{
    logW=new HGLogWidget("zh");
    // connect(pluginDetector_widget,SIGNAL(sendMsg2Manager(int)),this,SLOT(recvMsgFromUI(int)));

    QWidget* _widget=logW;
    return _widget;
}
void LogPluginDetector::recvMsgFromUI(int value)
{
    emit sendMsg2Manager(_Plugin_Name,"main",QString::number(value));
}
std::map<std::string,std::vector<std::string>> LogPluginDetector::moduleNodes()
{
