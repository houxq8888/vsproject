#include "ebalanceplugindetector.h"
...
EbalancePluginDetector::EbalancePluginDetector(QObject* parent)
{
    _Plugin_Name="EBalance";
}
std::string EbalancePluginDetector::pluginName()
{
    std::string name=_Plugin_Name.toStdString();
    return name;
}
QWidget* EbalancePluginDetector::getWidget()
{
    ebalanceW=new HGEBalanceWidget("zh");
    // connect(pluginDetector_widget,SIGNAL(sendMsg2Manager(int)),this,SLOT(recvMsgFromUI(int)));

    QWidget* _widget=ebalanceW;
    return _widget;
}
void EbalancePluginDetector::recvMsgFromUI(int value)
{
    emit sendMsg2Manager(_Plugin_Name,"main",QString::number(value));
}
std::map<std::string,std::vector<std::string>> EbalancePluginDetector::moduleNodes()
{
