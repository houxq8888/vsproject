#include "reagentplugindetector.h"
...
ReagentPluginDetector::ReagentPluginDetector(QObject* parent)
{
    _Plugin_Name="Reagent";
}
std::string ReagentPluginDetector::pluginName()
{
    std::string name=_Plugin_Name.toStdString();
    return name;
}
QWidget* ReagentPluginDetector::getWidget()
{
    reagentManageW=new HGReagentManageWidget("zh");
    // connect(pluginDetector_widget,SIGNAL(sendMsg2Manager(int)),this,SLOT(recvMsgFromUI(int)));

    QWidget* _widget=reagentManageW;
    return _widget;
}
void ReagentPluginDetector::recvMsgFromUI(int value)
{
    emit sendMsg2Manager(_Plugin_Name,"main",QString::number(value));
}
std::map<std::string,std::vector<std::string>> ReagentPluginDetector::moduleNodes()
{
