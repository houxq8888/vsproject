...
LoginPluginDetector::LoginPluginDetector(QObject* parent)
{
    _Plugin_Name="Login";
}
std::string LoginPluginDetector::pluginName()
{
    std::string name=_Plugin_Name.toStdString();
    return name;
}
QWidget* LoginPluginDetector::getWidget()
{
    loginW=new HGLoginWidget("zh");
    // connect(pluginDetector_widget,SIGNAL(sendMsg2Manager(int)),this,SLOT(recvMsgFromUI(int)));

    QWidget* _widget=loginW;
    return _widget;
}
void LoginPluginDetector::recvMsgFromUI(int value)
{
    emit sendMsg2Manager(_Plugin_Name,"main",QString::number(value));
}
std::map<std::string,std::vector<std::string>> LoginPluginDetector::moduleNodes()
{
