#include "scannerplugindetector.h"
...
ScannerPluginDetector::ScannerPluginDetector(QObject* parent)
{
    _Plugin_Name="Scanner";
}
std::string ScannerPluginDetector::pluginName()
{
    std::string name=_Plugin_Name.toStdString();
    return name;
}
QWidget* ScannerPluginDetector::getWidget()
{
    scannerW=new ScannerWidget("zh");
    // connect(pluginDetector_widget,SIGNAL(sendMsg2Manager(int)),this,SLOT(recvMsgFromUI(int)));

    QWidget* _widget=scannerW;
    return _widget;
}
void ScannerPluginDetector::recvMsgFromUI(int value)
{
    emit sendMsg2Manager(_Plugin_Name,"main",QString::number(value));
}
std::map<std::string,std::vector<std::string>> ScannerPluginDetector::moduleNodes()
{
