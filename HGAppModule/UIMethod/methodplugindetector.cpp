#include "methodplugindetector.h"
...
MethodPluginDetector::MethodPluginDetector(QObject* parent)
{
    _Plugin_Name="Method";
}
std::string MethodPluginDetector::pluginName()
{
    std::string name=_Plugin_Name.toStdString();
    return name;
}
QWidget* MethodPluginDetector::getWidget()
{
    methodManageW=new HGMethodManageWidget("zh");
    // connect(pluginDetector_widget,SIGNAL(sendMsg2Manager(int)),this,SLOT(recvMsgFromUI(int)));

    QWidget* _widget=methodManageW;
    return _widget;
}
void MethodPluginDetector::recvMsgFromUI(int value)
{
    emit sendMsg2Manager(_Plugin_Name,"main",QString::number(value));
}
std::map<std::string,std::vector<std::string>> MethodPluginDetector::moduleNodes()
{
