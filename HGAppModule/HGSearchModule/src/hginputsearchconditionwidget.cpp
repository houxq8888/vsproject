    ...
    m_maxRange(maxRange)
{
    ...
    m_searchLayout->addWidget(m_clearSearchLabel,1,2);
...
void HGInputSearchConditionWidget::slotSearch(){
    ...
    bool hasKeyword = !m_keyEdit->text().isEmpty();
    bool hasTimeFrom = !m_timeRangeFromEdit->text().isEmpty();
    bool hasTimeTo = !m_timeRangeToEdit->text().isEmpty();
    
    if (hasKeyword || (hasTimeFrom && hasTimeTo)){
        emit signalSearch();
