#include "channelplugindetector.h"
...
ChannelPluginDetector::ChannelPluginDetector(QObject* parent)
{
    _Plugin_Name="SetChannel";
}
std::string ChannelPluginDetector::pluginName()
{
    std::string name=_Plugin_Name.toStdString();
    return name;
}
QWidget* ChannelPluginDetector::getWidget()
{
    channelSetW=new HGChannelSetWidget("zh");
    // connect(pluginDetector_widget,SIGNAL(sendMsg2Manager(int)),this,SLOT(recvMsgFromUI(int)));

    QWidget* _widget=channelSetW;
    return _widget;
}
void ChannelPluginDetector::recvMsgFromUI(int value)
{
    emit sendMsg2Manager(_Plugin_Name,"main",QString::number(value));
}
std::map<std::string,std::vector<std::string>> ChannelPluginDetector::moduleNodes()
{
