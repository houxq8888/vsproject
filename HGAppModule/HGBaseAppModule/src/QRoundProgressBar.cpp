...
QRoundProgressBar::QRoundProgressBar(const std::string& name,QWidget *parent) :
    QWidget(parent),
    m_name(name),
    m_min(0), m_max(100),
    m_value(25),
...
void QRoundProgressBar::drawText(QPainter &p, const QRectF &innerRect, double innerRadius, double value)
{
    ...
    // !!! to revise
    QFont f(font());
    f.setPixelSize(innerRadius * qMax(0.25, (0.35 - (double)m_decimals * 0.08))); // 0.05
    p.setFont(f);
    ...
    f.setPixelSize(innerRadius * 0.1);
    p.setFont(f);
    p.drawText(QPoint(0,innerRadius * 0.1),QString::fromStdString(m_name));
}
