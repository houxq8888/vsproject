                           ...
                           int L, int nLeafs, std::string desc_name)
{
    ...
    desc_name.resize(ns);
    printf("111123qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq\n");
    for(int i = 0; desc_name.c_str()[i] != '\0'; i++) {
        printf("%c", desc_name.c_str()[i]);
    }
    printf(" vocname============sss============================================\n");
    std::strcpy(_params._desc_name_, desc_name.c_str());
    printf("23qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq\n");
    ...
    _data = std::unique_ptr<char[], decltype(&AlignedFree)>((char*)AlignedAlloc(_params._aligment, (int)_params._total_size), &AlignedFree);
...
{
    memcpy(&_params, &voc._params, sizeof(params));
    _data = std::unique_ptr<char[], decltype(&AlignedFree)>((char*)AlignedAlloc(_params._aligment, (int)_params._total_size), &AlignedFree);
...
void Vocabulary::transform(const std::vector<Descriptor> &features, int level, fBow &result, fBow2 &result2, std::vector<std::vector<uint32_t>>& desc_node) const
{
    ...
    if (features.empty() || CV_8UC1 != _params._desc_type || sizeof(Descriptor) != size_t(_params._desc_size))
    {
        return;
    }
    ...
    if (_params._desc_type == CV_8UC1)
    {
        ...
        if (cpu_info->HW_x64)
        {
            if (_params._desc_size == 32)
                _transform2<L1_32bytes>(features, level, result, result2, desc_node);
            //full akaze
            else if ( _params._desc_size == 61 && _params._aligment % 8 == 0)
                _transform2<L1_61bytes>(features, level, result, result2, desc_node);
            //generic
            else
                _transform2<L1_x64>(features, level, result, result2, desc_node);
        }
        else  _transform2<L1_x32>(features, level, result, result2, desc_node);
    }
    else if (_params._desc_type == CV_32FC1)
    {
        if ( cpu_info->isSafeAVX() && _params._aligment % 32 == 0) //AVX version
        {
            if ( _params._desc_size == 256)  _transform2<L2_avx_8w>(features, level, result, result2, desc_node); //specific for surf 256 bytes
            else  _transform2<L2_avx_generic>(features, level, result, result2, desc_node); //any other
        }
        if ( cpu_info->isSafeSSE() && _params._aligment % 16 == 0) //SSE version
        {
            if ( _params._desc_size == 256) _transform2<L2_sse3_16w>(features, level, result, result2, desc_node); //specific for surf 256 bytes
            else _transform2<L2_se3_generic>(features, level, result, result2, desc_node); //any other
        }
        //generic version
        _transform2<L2_generic>(features, level, result, result2, desc_node);
    }
    ...
    if (norm > 0.0)
    {
        ...
        for (auto  &e : result) e.second *= static_cast<float>(inv_norm);
    }
}

void Vocabulary::transform_with_desc_nodes(const std::vector<Descriptor>& features,
    const std::vector<std::vector<uint32_t>>& desc_nodes, int level, fBow& result, fBow2& result2) const
{
    ...
    {
        if (_params._desc_size == 32)
            _transform3<L1_32bytes>(features, desc_nodes, level, result, result2);
        //full akaze
        else if (_params._desc_size == 61 && _params._aligment % 8 == 0)
            _transform3<L1_61bytes>(features, desc_nodes, level, result, result2);
        //generic
        else
            _transform3<L1_x64>(features, desc_nodes, level, result, result2);
    }
    else
    {
        _transform3<L1_x32>(features, desc_nodes, level, result, result2);
    }
...
fBow Vocabulary::transform(const std::vector<Descriptor> &features) const
{
    //if (features.rows == 0) throw std::runtime_error("Vocabulary::transform No input data");
    //if (features.type() != _params._desc_type) throw std::runtime_error("Vocabulary::transform features are of different type than vocabulary");
    //if (features.cols *  features.elemSize() != size_t(_params._desc_size)) throw std::runtime_error("Vocabulary::transform features are of different size than the vocabulary ones");
