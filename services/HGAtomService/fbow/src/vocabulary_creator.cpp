...
void VocabularyCreator::create(Vocabulary &Voc, const std::vector<Descriptor> &features, const std::string &desc_name, Params params)
{
    std::vector<std::vector<Descriptor>> vfeatures(1);
    vfeatures[0] = features;
    create(Voc, vfeatures, desc_name, params);
}

void VocabularyCreator::create(Vocabulary &Voc, const std::vector<std::vector<Descriptor>> &features, const std::string &desc_name, Params params)
{
    ...
    if (_params.nthreads > 1)
    {
        ...
        for (uint32_t i = 0; i < _params.nthreads; i++)
...
//        std::cout<<n.first<<" ";std::cout<<std::endl;
    ...
    printf("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq\n");
}
...
void  VocabularyCreator::createLevel(  int parent, int curL, bool recursive)
{
    ...
    std::vector<Descriptor> center_features;
    ...
    else
    {
        ...
        {
            id_assigments.create( parent * _params.k + 1 + (uint32_t)i, (uint32_t)findices.size() / _params.k);
            assigments_ref.push_back( id_assigments[ parent * _params.k + 1 + (uint32_t)i]);
        }
        ...
        assignToClusters(findices, center_features, assigments_ref /*,parent==0*/);
        if (_params.verbose) std::cerr << "Cluster created :" << parent << " " << curL << std::endl;
    ...
    new_nodes.reserve(center_features.size());
    {
        for (size_t c = 0; c < center_features.size(); c++)
            new_nodes.push_back(Node(parent * _params.k + 1 + (uint32_t)c, parent, center_features[c], findices.size() == center_features.size() ? findices[c] : (std::numeric_limits<uint32_t>::max)()));
    }
    ...
    if ( ( (_params.L != -1 && curL < (_params.L - 1)) || _params.L == -1)  && assigments_ref.size() > 0)
    {
        ...
        else //parallel mode (multiple theads)
        {
            ...
            for (size_t i = 0; i < new_nodes.size(); i++)
                ParentDepth_ProcesQueue.push(std::make_pair(parent * _params.k + 1 + (int)i, curL + 1));
        }
    }
}
...
std::vector<uint32_t> VocabularyCreator::initialClusterCentersKmpp(const std::vector<uint32_t> &findices)
{
    ...
    while ((int)centers.size() < _params.k)
    {
        ...
        double dist_sum = std::accumulate(findices.begin(), findices.end(), 0.0f, [&](float acc, const unsigned fid) { return acc + _features(fid).m_Dist; });
        if (dist_sum > 0)
        {
            double cut_d;
            do
...
void VocabularyCreator::assignToClusters( const std::vector<uint32_t> &findices,  const std::vector<Descriptor> &center_features, std::vector<vector_sptr> &assigments, bool omp)
{
    ...
    if (omp  )
    {
        ...
        for (int i = 0; i < int(findices.size()); i++)
        {
            ...
            for (size_t ci = 1; ci < center_features.size(); ci++)
            {
                ...
                if (dist < center_dist_min.second) center_dist_min = std::make_pair((uint32_t)ci, dist);
            }
...
//            assigments[center_dist_min.first]->push_back(fi);
    ...
    else
    {
        ...
        for (auto fi : findices)
        {
            const auto &feature = _features[fi];
            std::pair<uint32_t, float> center_dist_min(0, dist_func(center_features[0], feature));
            for (uint32_t ci = 1; ci < center_features.size(); ci++)
            {
                float dist = dist_func(center_features[ci], feature);
...
std::vector<Descriptor> VocabularyCreator::recomputeCenters(  const std::vector<vector_sptr> &assigments, bool omp)
{
...
Descriptor VocabularyCreator::meanValue_binary( const std::vector<uint32_t>  &indices)
{
    ...
    for (auto i : indices)
    {
        const unsigned char *p = _features[i].data;
...
void VocabularyCreator::convertIntoVoc(Vocabulary &Voc,  std::string  desc_name)
{
    ...
    printf("123qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq\n");
    ...
    { 
        if (!node.second.isLeaf())
        {
            ...
            for (size_t c = 0; c < node.second.children.size(); c++)
            {
                ...
                if (child.isLeaf())   binfo.getBlockNodeInfo((int)c)->setLeaf(child.feat_idx, child.weight);
                else
                {
...
float VocabularyCreator::distance_hamming_generic(const Descriptor &a, const Descriptor &b)
{
    uint64_t ret = 0;
    const uchar *pa = a.data; // a & b are actually CV_8U
    const uchar *pb = b.data;
    for (int i = 0; i < DESC_SIZE; i++, pa++, pb++)
    {
        uchar v = (*pa) ^ (*pb);
#ifdef __GNUG__
...
//for orb
float VocabularyCreator::distance_hamming_32bytes(const Descriptor &a, const Descriptor &b)
{
    const uint64_t *pa = (uint64_t *)a.data; // a & b are actually CV_8U
    const uint64_t *pb = (uint64_t *)b.data;
    #ifdef _ON_WIN_
    return float(__popcnt64(pa[0] ^ pb[0]) + __popcnt64(pa[1] ^ pb[1]) + __popcnt64(pa[2] ^ pb[2]) + __popcnt64(pa[3] ^ pb[3]));
    #else
    return float(__builtin_popcountl(pa[0] ^ pb[0]) + __builtin_popcountl(pa[1] ^ pb[1]) + __builtin_popcountl(pa[2] ^ pb[2]) + __builtin_popcountl(pa[3] ^ pb[3]));
    #endif
}
