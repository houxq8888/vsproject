...
{
#define DESC_SIZE 32
#define DESC_TYPE CV_8U

struct /*FBOW_API*/ Descriptor
{
    unsigned char data[DESC_SIZE];
    ...
    {
        memcpy(data, desc.data, sizeof(data));
    }
};
...
public:
    ...
    fBow transform(const std::vector<Descriptor> &features) const;
    void transform(const std::vector<Descriptor> &features, int level, fBow &result, fBow2 &result2, std::vector<std::vector<uint32_t>> &desc_node) const;
    void transform_with_desc_nodes(const std::vector<Descriptor>& features,
        const std::vector<std::vector<uint32_t>>& desc_nodes, int level, fBow& result, fBow2& result2) const;
...
//private:
    ...
    struct params
    {
        char _desc_name_[150];//descriptor name. May be empty  //dingjiangang change  50
    ...
    struct block_node_info
    {
        ...
        float weight;
        inline bool isleaf()const {return ( id_or_childblock & 0x80000000) != 0;}
    ...
    struct Block
    {
        ...
        using add_const_t = typename std::add_const<_Tp>::type;
        ...
        inline void setFeature(int i, const Descriptor &feature)
        { memcpy(_blockstart + _feature_off_start + i * _desc_size_bytes_wp, feature.data, sizeof(Descriptor)); }
        inline  void getFeature(int i, Descriptor& feature) 
        { memcpy(feature.data, _blockstart + _feature_off_start + i * _desc_size_bytes, _desc_size_bytes); }
        inline void getFeature(int i, Descriptor &feature) const
        { memcpy(feature.data, _blockstart + _feature_off_start + i * _desc_size_bytes, _desc_size_bytes); }
    ...
    struct  L1_x64: public Lx<uint64_t, uint64_t, 8>
    {
        inline uint64_t computeDist(const uint64_t *feat_ptr)
        {
            uint64_t result = 0;
            #ifdef _ON_WIN_
            //for (int i = 0; i < _nwords; ++i ) result += __popcnt64(feat_ptr[i] ^ feature[i]);
            #else
            for (int i = 0; i < _nwords; ++i ) result += __builtin_popcountl(feat_ptr[i] ^ feature[i]);
            #endif
            return result;
        }
    };
    ...
    struct  L1_x32: public Lx<uint32_t, uint32_t, 8>
    {
        inline uint32_t computeDist(const uint32_t *feat_ptr)
        {
            uint32_t result = 0;
            #ifdef _ON_WIN_
            //for (int i = 0; i < _nwords; ++i ) result += __popcnt(feat_ptr[i] ^ feature[i]);
            #else
            for (int i = 0; i < _nwords; ++i ) result += __builtin_popcount(feat_ptr[i] ^ feature[i]);
            #endif
            return result;
        }
    ...
    fBow  _transform(const std::vector<Descriptor> &features) const
    {
        Computer comp;
        ...
        fBow  result;
        std::pair<DType, uint32_t> best_dist_idx((DType)(std::numeric_limits<uint32_t>::max)(), 0); //minimum distance found
        const block_node_info *bn_info;
        int feature_num = (int)features.size();
        for (int cur_feature = 0; cur_feature < feature_num; cur_feature++)
        {
            comp.startwithfeature((TData *)features[cur_feature].data);
            ...
            do
            {
                ...
                best_dist_idx.first = (DType)(std::numeric_limits<uint32_t>::max)();
    ...
    template<typename Computer>
    void  _transform2(const std::vector<Descriptor> &features, uint32_t storeLevel, fBow &r1, fBow2 &r2, std::vector<std::vector<uint32_t>>& desc_node) const
    {
        Computer comp;
        comp.setParams(_params._desc_size, (int)_params._desc_size_bytes_wp);
        using DType = typename Computer::DType; //distance type
        ...
        for (int cur_feature = 0; cur_feature < feature_num; cur_feature++)
        {
            ...
            do
            {
                ...
                for (int cur_node = 0; cur_node < c_block.getN(); cur_node++)
                {
                    DType d = (DType)comp.computeDist(c_block.getFeature<TData>(cur_node));
                    if (d < best_dist_idx.first) best_dist_idx = std::make_pair(d, cur_node);
                }
                ...
                bn_info = c_block.getBlockNodeInfo(best_dist_idx.second);
                node_indices.push_back(best_dist_idx.second);
            ...
            while ( !bn_info->isleaf() && bn_info->getId() != 0);
            desc_node.push_back(node_indices);
        }
    }

    template<typename Computer>
    void  _transform3(const std::vector<Descriptor>& features, const std::vector<std::vector<uint32_t>>& desc_nodes, uint32_t storeLevel, fBow& r1, fBow2& r2) const
    {
        Computer comp;
        comp.setParams(_params._desc_size, (int)_params._desc_size_bytes_wp);
        using DType = typename Computer::DType; //distance type
        ...
        for (const auto& cur_desc_node : desc_nodes)
        {
            Block<const char> c_block = getBlock(0);
            for (const auto& node_id : cur_desc_node)
            {
                bn_info = c_block.getBlockNodeInfo(node_id);
                if (bn_info->isleaf())
                {
                    r1[bn_info->getId()] += bn_info->weight;
                    break;
                ...
                {
                    setBlock(bn_info->getId(), c_block);
                }
            }
        }
    }
