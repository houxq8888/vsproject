...
public:
    ...
    void create(Vocabulary &Voc, const std::vector<std::vector<Descriptor>> &features, const std::string &desc_name, Params params);
    void create(Vocabulary &Voc, const std::vector<Descriptor> &features, const std::string &desc_name, Params params);
private:
    Params _params;
    struct feature_info
    {
    ...
    struct FeatureInfo
    {
        void create(const std::vector<std::vector<Descriptor>> &f)
        {
            features = (std::vector<std::vector<Descriptor>>*)&f;
            uint32_t _size = 0;
            for (auto &m : *features) _size += (uint32_t)m.size();
            fInfo.clear();
            fInfo.reserve(_size);
            for (size_t midx = 0; midx < features->size(); midx++)
            {
                auto nrows = features->at(midx).size();
                for (int i = 0; i < nrows; i++)
                    fInfo.push_back(feature_info((uint32_t)midx, i));
            }
        }
        size_t size()const {return fInfo.size();}
        template<typename T>
        inline T*getFeaturePtr(uint32_t i) {const auto &idx = fInfo[i]; return (T *)features->at(idx.midx).at(idx.fidx).data;}
        inline Descriptor& operator[](uint32_t i) {const auto &idx = fInfo[i]; return features->at(idx.midx).at(idx.fidx);}
    ...
    Descriptor meanValue_binary( const std::vector<uint32_t>  &indices);
    ...
    {
        Node() {}
        Node(uint32_t Id, uint32_t Parent, const Descriptor &Feature, uint32_t Feat_idx = (std::numeric_limits<uint32_t>::max)() ): id(Id), parent(Parent), feature(Feature), feat_idx(Feat_idx)
        {
        ...
        Descriptor feature;//feature of this node
    ...
    void assignToClusters(const std::vector<uint32_t> &findices, const std::vector<Descriptor> &center_features, std::vector<vector_sptr> &assigments, bool omp = false);
    std::vector<Descriptor>  recomputeCenters(const std::vector<vector_sptr> &assigments, bool omp = false);
...
#endif
    static float distance_hamming_generic(const Descriptor &a, const Descriptor &b);
    static float distance_hamming_32bytes(const Descriptor &a, const Descriptor &b);
    std::function<float(const Descriptor &a, const Descriptor &b)> dist_func;
